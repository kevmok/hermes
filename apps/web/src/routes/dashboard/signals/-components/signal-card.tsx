...
function getTotalTradeSize(
  ...
  if (Array.isArray(trade)) {
    return trade.reduce((sum, t) => sum + t.size, 0);
  }
  return trade.size;
}
...
export function SignalCard({ signal, index = 0, onSelect }: SignalCardProps) {
  ...
  // Calculate consensus bar width
  const consensusWidth = Math.min(100, signal.consensusPercentage);
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="relative">
              ...
              />
              <div
                className={`relative flex h-11 w-11 items-center justify-center rounded-xl bg-linear-to-br ${config.bgGradient} ${config.borderColor} border shadow-lg ${config.glowColor}`}
              >
                <HugeiconsIcon
                  icon={config.icon}
                  size={22}
