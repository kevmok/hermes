...
function SignalHistoryPage() {
  ...
  const { data: signalsWithOutcomes, isLoading } = useQuery(
    convexQuery(api.performanceMetrics.getSignalsWithOutcomes, {
      limit: 100,
      onlyEvaluated:
        tab === 'evaluated' || tab === 'correct' || tab === 'incorrect',
    }),
  );
  ...
  const counts = {
    ...
    evaluated:
      signalsWithOutcomes?.filter((s) => s.isCorrect !== null).length ?? 0,
    correct:
      signalsWithOutcomes?.filter((s) => s.isCorrect === true).length ?? 0,
    incorrect:
      signalsWithOutcomes?.filter((s) => s.isCorrect === false).length ?? 0,
  };
