...
const fetchJsonArray = <T>(
  url: string,
): Effect.Effect<T[], PolymarketApiError> =>
  Effect.tryPromise({
    try: async () => {
      const response = await fetch(url);
...
export const getMarketBySlug = (
  ...
  fetchJson<Market>(`${GAMMA_API}/markets/slug/${encodeURIComponent(slug)}`);
...
export const api = {
  ...
  listEvents: (params: ListEventsParams = {}) =>
    Effect.runPromise(listEvents(params)),
  ...
  listMarkets: (params: ListMarketsParams = {}) =>
    Effect.runPromise(listMarkets(params)),
  ...
  getPortfolioValue: (user: string) =>
    Effect.runPromise(getPortfolioValue(user)),
